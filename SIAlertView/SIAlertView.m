...
#define KEYBOARD_MARGIN_IPHONE_PORTRAIT 107
#define KEYBOARD_MARGIN_IPHONE_LANDSCAPE 76
#define KEYBOARD_MARGIN_IPAD_PORTRAIT 100
...
//        height = self.alertView.frame.size.height > self.alertView.frame.size.width ? self.alertView.frame.size.height : self.alertView.frame.size.width;
//    else
//        height = self.alertView.frame.size.height < self.alertView.frame.size.width ? self.alertView.frame.size.height : self.alertView.frame.size.width;
//    float containerHeight = self.alertView.containerView.frame.size.height;
//    float keyBoardHeight = kbSize.height < kbSize.width ? kbSize.height : kbSize.width;
//    
//    float distanceFromBottom = height - (self.alertView.containerView.frame.origin.y + containerHeight);
//
//    float margin = keyBoardHeight - distanceFromBottom;
//
    float margin = 0;
    UIInterfaceOrientation orientation = [UIApplication sharedApplication].statusBarOrientation;
    //Selection de la marge en fonction du device et de l'orientation
    if([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone)
    {
        if(orientation == UIInterfaceOrientationPortrait)
        
            margin = KEYBOARD_MARGIN_IPHONE_PORTRAIT;
        
        else
            margin = KEYBOARD_MARGIN_IPHONE_LANDSCAPE;
    }
    else
    {
        if (UIInterfaceOrientationIsLandscape(orientation)) {
            margin = KEYBOARD_MARGIN_IPAD_PORTRAIT;
        }
    }
    
    if(margin > 0)
    {  
        CGAffineTransform translate = CGAffineTransformMakeTranslation(0.0, -margin);

        [UIView animateWithDuration:0.5 delay:0 options:UIViewAnimationCurveEaseInOut animations:^{
            [self.alertView.containerView setTransform:translate];
        } completion:nil];
    }
...
//    CGSize kbSize = [[info objectForKey:UIKeyboardFrameBeginUserInfoKey] CGRectValue].size;
//    
//    float distanceFromBottom = self.alertView.frame.size.height - (self.alertView.containerView.frame.origin.y + self.alertView.containerView.frame.size.height);
//    float margin = distanceFromBottom - kbSize.height;
//    if(margin > 0)
//    {
//        CGAffineTransform translate = CGAffineTransformMakeTranslation(0.0, margin);
//        
//        [UIView animateWithDuration:[[info objectForKey:UIKeyboardAnimationDurationUserInfoKey] floatValue] delay:0 options:[info objectForKey:UIKeyboardAnimationCurveUserInfoKey] animations:^{
//            [self.alertView.containerView setTransform:translate];
//        } completion:nil];
//    }
}
...
- (id)initWithTitle:(NSString *)title andMessage:(NSString *)message
{
	...
	if (self) {
		_title = title;
        _message = message;
        _empecherFermeture = false;
		self.items = [[NSMutableArray alloc] init];
	}
	return self;
}
- (id)initWithTitle:(NSString *)title andTextFields:(NSArray *)textFields
{
    self = [super init];
	if (self) {
		_title = title;
...
- (void)validateLayout
{
    ...
    self.layoutDirty = NO;
#if DEBUG_LAYOUT
    DLog(@"%@, %@", self, NSStringFromSelector(_cmd));
#endif
    ...
    if([_textFields count] != 0)
    {
        for (UITextField* textField in _textFields) {
            y += 14;
            textField.frame = CGRectMake(CONTENT_PADDING_LEFT, y, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, 30);
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2) {
            ...
            button = self.buttons[1];
            button.frame = CGRectMake(CONTENT_PADDING_LEFT + width + GAP, y, width, BUTTON_HEIGHT);
            //Troisième bouton stacké en dessous
...
- (CGFloat)preferredHeight
{
    ...
    if (self.items.count > 0) {
        ...
        {
            height += 2* BUTTON_HEIGHT + GAP;
        }
        else {
            height += (BUTTON_HEIGHT + GAP) * self.items.count - GAP;
            if (self.buttons.count > 3 && ((SIAlertItem *)[self.items lastObject]).type == SIAlertViewButtonTypeCancel) {
...
- (void)setupContainerView
{
    ...
    self.containerView.backgroundColor = _cvBackgroundColor;
...
- (void)updateTitleLabel
{
	if (self.title) {
		if (!self.titleLabel) {
            ...
            self.titleLabel.shadowColor = [UIColor colorWithWhite:0.8 alpha:1];
...
- (void)setCvBackgroundColor:(UIColor *)backgroundColor
{
    if (_cvBackgroundColor == backgroundColor) {
        return;
    }
    _cvBackgroundColor = backgroundColor;
